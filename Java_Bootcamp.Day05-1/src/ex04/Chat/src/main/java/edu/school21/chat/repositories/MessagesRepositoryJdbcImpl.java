...
public class MessagesRepositoryJdbcImpl implements MessagesRepository {
    ...
    public void save(Message message) {
        if (findUserById(message.getAuthor().getId()) == null)
    ...
    public void update(Message message) {
        String query = "UPDATE chat.message SET author = ?, room = ?, text = ?, date_time = ? WHERE id = ?";
        try (PreparedStatement statement  = dataSource.getConnection().prepareStatement(query)) {
            if (message.getAuthor() == null || findUserById(message.getAuthor().getId()) == null)
                throw new NotSavedSubEntityException("Author id not found");
            if (message.getChatRoom() == null || findRoomById(message.getChatRoom().getId()) == null)
